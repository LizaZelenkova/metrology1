...
namespace Labametr1
{
    static class MetrikHolsteda
    {
        ...
        {
            string pattern = @" \+ | ={1,2} | \*{1,2} | [<>%-/&^|] | <= | >= | >> | << | =~ | \! | !~ | case | != | \+= | && | if | ?: | ? | unless | and | xor | or |[A-Za-z]\w+\(|\+{2}|\-{2}|%]";
        ...
        {
            int programVolume = (int)((allOperands + allOperators)*Math.Log(uniqueOperands+uniqueOperators,2));
            ...
            float levelOfProgram = (float)potentionalVolumeOfProgram / programVolume;
            float intellectualContentOfAlgorithm = levelOfProgram * programVolume;
            ...
            Console.WriteLine("Уникальные операторы:{0}", uniqueOperators);
            Console.WriteLine("Уникальные операнды:{0}", uniqueOperands);
            ...
            Console.WriteLine("Объем программы:{0}", programVolume);
